    ...
    try {
      ...
      for (const file of req.files) {
        ...
        try {
          ...
          // Process in background with autonomous agent
          setImmediate(async () => {
            try {
              console.log(`Starting autonomous agent analysis for program: ${parsedProgram.name}`);
              
              // Create autonomous agent instance
              const agent = new COBOLDocumentationAgent(`session_${Date.now()}`, 'system');
              
              // Set user preferences for autonomous processing
              agent.setUserPreference('detailLevel', 'high');
              agent.setUserPreference('audience', 'technical');
              agent.setUserPreference('aiProvider', 'anthropic');
              agent.setUserPreference('analysisDepth', 'comprehensive');
              
              // Let the agent autonomously analyze the COBOL structure
              console.log('Agent analyzing COBOL structure...');
              const structuredData = await agent.analyzeCobolStructure(sourceCode, program.id);
              console.log('Agent completed autonomous analysis');
              
              // Generate documentation using the autonomous agent
              console.log('Agent generating comprehensive documentation...');
              const documentation = await agent.generateDocumentation(structuredData);
              console.log('Agent completed documentation generation');
              
              // Evaluate documentation quality autonomously
              const quality = await agent.evaluateDocumentationQuality(documentation, program.id);
              console.log('Agent quality assessment:', quality.overallScore);
              
              // Update program with autonomous agent results
              console.log('Updating program with agent analysis...');
              await storage.updateProgram(program.id, {
                aiSummary: documentation.summary || structuredData.summary,
                complexity: documentation.complexity || structuredData.complexity,
                businessRules: documentation.businessRules || [],
                systemExplanation: documentation.systemExplanation || null,
                mermaidDiagram: documentation.mermaidDiagram || null,
                status: "completed",
              });
              console.log('Program updated successfully with autonomous agent results');
